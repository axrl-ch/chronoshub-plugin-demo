<template>
  <main v-if="page" style="max-width:720px; margin:0 auto; padding:2rem; font-family:system-ui,sans-serif;">
    <nav style="margin-bottom:1.5rem; font-size:0.875rem; color:#666;">
      ...
      <a href="/connectors" style="color:#666; text-decoration:none;">Connectors</a> /
    ...
    <table v-if="page.version || page.category" style="border-collapse:collapse; font-size:0.9rem; margin-bottom:1.5rem;">
      <tr v-if="page.category">
        <td style="color:#666; padding:0.3rem 1.5rem 0.3rem 0;">Category</td>
        <td style="text-transform:capitalize;">{{ page.category }}</td>
      </tr>
    ...
    <div v-if="page.requiredScopes?.length">
      <h2 style="font-size:1rem; font-weight:600; margin:0 0 0.5rem;">Required permissions</h2>
      <ul style="margin:0; padding-left:1.25rem; color:#444; font-size:0.9rem; line-height:1.8;">
        <li v-for="s in page.requiredScopes" :key="s"><code>{{ s }}</code></li>
      </ul>
    </div>
  </main>
</template>
